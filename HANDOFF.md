...
- 安全封條：內容 DB SHA-256 `ff407d384b33d95c82ade5923f6ab174182cd08d4a7194e48d0e8e623130fef0`；publish queue、daily runs、publish packages 均不存在，未登入新帳號、未建 Meta container、未發布。下一個【使用者操作】前須取得兩個非敏感預期 username，寫入 ignored env，再開新臨時網址；一次只引導一個 Meta callback 設定，禁止索取 secret／token。

- 2026-09-04 使用者核准「共幾局」後，隔離分支 `codex/publish-qa-label`（既有 `.worktrees/leaguepedia-diagnostics`）完成 PostMatchReadFrame 最小修正：讀取正整數 gameCount，缺值不猜賽制，GAME_FLOW／比分／樣式不變。真實 JSX regression 3/3，兩次 red→green；doctor、641 tests／635 pass／6 skip／0 fail、build 與 browser 5/5 通過。`npm audit` 服務逾時，日誌 `/tmp/lol-label-audit.log`，尚未合併 main／push；原本機服務仍是舊標頭，勿稱已上線。
- 新真實 HLE preview 位於隔離工作區 `public/renders/render_1788502413340.mp4`，SHA-256 `262c640b6ac52764287a4c694ba0a735c9ee0b9feae6f959e3d3ae929a7cffb8`；媒體驗證通過、25.045333 秒、無發布工作。最後一幕 `/tmp/lol-label-final.png`（540×960）確認「共 5 局」與比分沒有重疊、HLE 隊徽正常。網頁兩輪最終截圖主專案 `.screenshots/label-round2-desktop.png`／`label-round2-mobile.png`。計畫與證據在隔離分支 `docs/superpowers/plans/2026-09-04-publish-qa-label.md`。
- 發布 readiness 最新診斷：IG／Threads 各一次唯讀帳號回覆 HTTP 400/code 190，錯誤明確含 expired，已確認授權過期。既有 callback 與 media 兩個臨時域名均 ENOTFOUND。尚未改 token／env／開隧道／發布，需確認可用固定網域或核准臨時方案，再安排平台 callback 登錄與逐步登入；見 docs/open-questions.md。禁止把重試發文當成授權恢復。

- 2026-09-04 產片到發布 QA：使用者選擇真實發布，但必須先列出影片／平台／帳號並取得確認；本次未送出。真實 UI 使用已保存 2026-09-02 T1 vs Hanwha Life Esports 2-3 snapshot，一次預覽請求約 5.2 分鐘完成，畫面顯示媒體驗證通過；`public/renders/render_1788501618033.mp4`，SHA-256 `29ef18f5a2da4713570ef760cc12b9d18940cc10f3446e42039ff288a4709e62`，ffprobe H.264/AAC、1080×1920、25.045333 秒，browser readyState 4。最後一幕 `/tmp/lol-publish-qa-final.png`（540×960）確認 HLE 隊徽可見，但 `PostMatchReadFrame.jsx` 硬編 BO3 與五局賽事矛盾，阻擋公開發布。
- 發布 QA 真實環境阻擋：Instagram／Threads 各一次唯讀帳號查詢均 HTTP 400、OAuthException code 190，無法核實帳號；未判定是過期、撤銷或其他授權原因。既有 public media URL HEAD 檢查無法連線。沒有建立平台容器／貼文／發布工作。既有 preflight 可接受影片與證據，但不代表內容賽制或外部帳號／取檔已就緒。隔離工作區發布相關測試 162/162 通過（`/tmp/lol-publish-qa-tests.log`）、瀏覽器流程 5/5 通過（`/tmp/lol-publish-qa-e2e.log`）；平台發布是模擬回應，不可當成真實發布成功。內容 DB SHA-256 仍 `ff407d384b33d95c82ade5923f6ab174182cd08d4a7194e48d0e8e623130fef0`，queue／daily runs／publish packages 皆不存在。本輪僅 QA 與紀錄，未修產品程式碼；待核准處理內容標籤、平台授權與公開媒體網址後，重新驗證並列出確定帳號／成品／貼文取得發布確認。

- 2026-09-04 HLE 勝方隊徽身分修正：本機 main `a8b46b5`。根因是素材 resolver 正確把 `Hanwha Life Esports` 解析為 `HLE`，最後一幕卻拿 canonical 縮寫與原全名直接比較。planner 現在保留原 series identity 與已驗證 crest 的配對，只接受唯一的本場隊伍匹配；不做第二次 manifest lookup，不改素材／前端／登入／發布行為。真實 HLE PNG regression 先重現原錯誤再綠，並涵蓋兩隊位置、Gen.G、canonical 名稱、空／未知／場外／歧義勝方。聚焦 12/12；分支 doctor、638 tests／632 pass／6 skip、build、render 6/6、browser 5/5 通過；主線驗證記錄 `/tmp/lol-crest-main-{doctor,coverage,build,render,e2e}.log`。
- HLE 真實保存 snapshot `scan-2026-09-02-28fac0de6f` 的 preview-only canary 成功：`public/renders/render_1788501212137.mp4`，SHA-256 `f1d5c745b1fa43ca538b5848370859ca320c0d7314d573a25bf6aee32a90a72b`，H.264/AAC、1080×1920、30fps、25.045333 秒、-17 LUFS、true peak -1.9 dBFS，媒體驗證通過。最後一幕自檢圖 `/tmp/lol-hle-final.png`（540×960）確實顯示 HLE 隊徽。未查 Leaguepedia，未產生發布工作；內容 DB hash 仍 `ff407d384b33d95c82ade5923f6ab174182cd08d4a7194e48d0e8e623130fef0`，publish stores 均不存在。另觀察到既有 BO3 標籤與五局比分不一致，與隊徽身分修正無關，未擅自改動。
- 主線 doctor、638 tests／632 pass／6 skip／0 fail、build、render 6/6、browser 5/5 均通過。遠端推送仍阻擋：本輪 branch/main 的 npm audit 均因服務逾時失敗，證據 `/tmp/lol-crest-audit.log`、`/tmp/lol-crest-main-audit.log`；不是漏洞發現，不可宣稱安全檢查通過。先恢復原本機網址，服務恢復後再完成安全閘門及 push；保留原有 package-lock.json／AGENTS.md／CLAUDE.md 使用者修改。

- **本輪收尾阻擋（2026-09-04）**：診斷提交 `fe1e964` 已在本機 main，尚未 push。分支 doctor／635 tests（629 pass、6 skip）／build／audit 0／render 6/6／browser 5/5 通過；main 重跑 doctor／同數測試／build／render 6/6／browser 5/5 通過，但 npm audit 服務 HTTP 503，後續單次 15 秒 bounded request 仍逾時。不是已知漏洞，不跳過安全閘門。證據 `/tmp/lol-diag-main-audit.log`、`/tmp/lol-diag-main-audit-bounded.log`；服務恢復後重跑 `npm audit --audit-level=high --fetch-retries=0 --fetch-timeout=15000`，通過才提交收尾文件並 push。原有使用者 lockfile metadata 修改不得提交。原網址恢復現有本機版本，不再發出 Leaguepedia 查詢；本次來源受控測試已用完一次授權。

- 2026-09-04 新增限流安全診斷（使用者核准）：`utils/leaguepediaApi.js` 僅對終止的 HTTP／MediaWiki 失敗寫入 `[Leaguepedia diagnostic]` 固定欄位，區分 transport status 與 payload error；不記任意本文、帳號、cookie、token、URL 或完整 headers。不更動重試／登入／快取／發布規則；未知 error code 歸 other，Retry-After 只接受有限長度整數，不能把它當作保證解封時間。既有登入錯誤路徑不在此輪擴充。
- 新測試 `tests/unit/leaguepediaDiagnostics.test.js` 以真實本機 HTTP server 覆蓋 429、HTTP 200 JSON ratelimit、503、未知 code、敏感字串排除與冷卻期間零新增診斷／零請求。兩個新紀錄入口均先紅後綠。首次完整測試遇到 Node v24.15.0 test-runner IPC deserialize failure，聚焦測試不重現；收集診斷但靜音無關 console.log 後完整覆蓋率通過（635 tests／629 pass／6 外部 skip／0 fail）。未變更產品輸出或降低測試門檻。
- 受控來源查詢已執行一次：確認主線 cooldown 到期後，以新程序正常 Bot 登入，對原 2026-09-02 篩選做 limit=1 Cargo 診斷；網路守衛阻止第一筆 Cargo 之後的任何請求。token／login／Cargo 均 HTTP 200，Cargo calls=1、rows=1。未跑完整掃描、未保存候選、未產片或發布；成功依既有行為清除 cooldown。這不能回溯證明先前拒絕的原因，因等待與新登入兩個條件同時改變；不可宣稱限流根因已修復。
- 施工計畫：`docs/superpowers/plans/2026-09-04-leaguepedia-diagnostics.md`。分支 `codex/leaguepedia-diagnostics` 保留原主線使用者 lockfile／AGENTS.md／CLAUDE.md 變更。依 inline 約定按 requesting-code-review 清單自審；完成後依既定規則整合 main、重跑閘門並推送，原網址不變。驗證日誌 `/tmp/lol-diag-{doctor,coverage-final,build,audit,render,e2e}.log`；主線收尾日誌使用 `/tmp/lol-diag-main-*`。
...
- 2026-09-03 使用者同意有限度的七天備援。`codex/leaguepedia-scan-cache` 已實作歷史 UTC 日期的 24 小時完整 snapshot 重用，僅 `LEAGUEPEDIA_RATE_LIMITED` 才回退七天內完整且日期／模式／語言／scope／resolved tournaments 一致的 snapshot；不使用今天／未來日期、不保存空資料作為備援。fallback 保留 scanId、createdAt、candidates，只在 sourceStatus 記錄 rate_limit 與原始 cachedAt，使預覽程序重啟後仍可讀取，其他 snapshot 保留一天預設期限。無效／未來 createdAt 拒絕讀取，明確 maxAgeMs 仍可收緊。
- 施工計畫 `docs/superpowers/plans/2026-08-30-leaguepedia-scan-cache.md` 已按實際預覽契約補齊。完成逐一紅→綠：精確保存資料匹配、零請求 fresh reuse、rate-limit fallback、真實檔案＋獨立程序讀回、非法时间拒絕、瀏覽器提示與原始 scanId 預覽。額外 guard tests 保護今天／未來、非限流錯誤、七天過期、完整性與 timestamp provenance。依先前 inline 選擇未派子代理，自行按 requesting-code-review 清單審查；沒有新依賴、沒有變動發布或素材安全閘門。
- 分支驗證：TDD doctor 通過；coverage 630 tests／624 pass／6 外部 skip／0 fail，line 94.36%、branch 80.98%、function 96.48%；Next build 通過，保留既知 3 個 portrait filesystem tracing warnings；Remotion QA 6/6、Playwright 5/5。日誌 `/tmp/lol-cache-coverage.log`、`/tmp/lol-cache-build.log`、`/tmp/lol-cache-renderqa.log`、`/tmp/lol-cache-e2e.log`。
- 桌面／手機兩輪自檢完成，最終圖 `.screenshots/scan-cache-round2/desktop.png`（1280×800）、`.screenshots/scan-cache-round2/mobile.png`（375×812）。層次、留白、字體、配色、對齊、響應式、空／載入／錯誤／focus 狀態與既有微動效均通過；保留現有深色霧金設計與 WorkflowStatus，不新增動效。實測手機 clientWidth／scrollWidth 均 375、console error／warning 0。
- 隔離 49762 服務以主線真實 snapshot `scan-2026-08-28-80402692da`（取得時間 2026-08-29T23:55:56.527Z）驗證；只在隔離副本設定一小時 cooldown 模擬限流，未改主線 cooldown。真實 POST candidates HTTP 200，保留 5 個候選；BNK FEARX vs HANJIN BRION 3-2 preview HTTP 200、媒體驗證通過、video readyState 4，25.045333 秒。檔案 `public/renders/render_1788468952603.mp4`（ignored），8,599,350 bytes、SHA-256 `7e362ec885530936ff667ed33f789bdf83fcf783aaea401351860c01a9e23a60`。以程式實際 Cargo log marker 先校準一筆已知樣本，再計數隔離 log，查詢數 0；測試涵蓋從真實 cooldown 走真實 reader，不只 mock。
- 主線內容 DB SHA-256 保持 `ff407d384b33d95c82ade5923f6ab174182cd08d4a7194e48d0e8e623130fef0`；兩邊 publish queue／esports-daily-runs／publish-packages 均不存在，隔離內容 DB 不存在。隔離 dev server 已停止，主線 49761 未重啟／換版。
- **整合阻擋**：2026-09-03 npm audit 在分支及原主線都報 2 個 high：browserslist <=4.28.6、fast-uri 3.0.0–3.1.5，皆有可用修補。原始報告 `/tmp/lol-cache-audit.log`、`/tmp/lol-cache-main-audit.json`。不是本次新增依賴導致，未執行 npm audit fix；依規則不合併、不 push、不部署，改動尚未提交，保留隔離工作區待核准這兩項原有套件修補。主工作區 package-lock.json 原有未提交修改（移除 libc metadata）與 untracked AGENTS.md／CLAUDE.md 均保留；不可將它們混入本次提交。
...
- 勝方隊徽背景分支完整品質閘門通過：TDD doctor；coverage 624 tests／618 pass／6 個明確外部 skip／0 fail，line 94.30%、branch 80.61%、function 96.13%；Next 26 routes build 通過，僅保留既知 3 個 `playerPortraitManifest.js` tracing warnings；audit 0 vulnerabilities、Remotion QA 6/6、Playwright 4/4。自我 code review 確認沒有固定隊伍／選手名稱、沒有新外部相依或發布副作用，缺勝方身分／證據／已核准隊徽仍維持硬阻擋。
- Fast-forward 合併 `main` 後重跑相同完整閘門亦全綠：624 tests／618 pass／6 skip／0 fail，line 94.30%、branch 80.56%、function 96.13%；Next 26 routes、既知 3 warnings、audit 0、Remotion QA 6/6、Playwright 4/4。branch coverage 的 0.05% 差異屬既有條件分支測量浮動，兩次均高於 80% 門檻且測試數／失敗數一致。
- 永久服務 `http://localhost:49761/` 清除 production build cache 後重新 bootstrap，HTTP 200 且 LaunchAgent running。正式網址重跑 2026-08-28 掃描並選 `LCK · BNK FEARX vs HANJIN BRION · 3-2`，預覽停留於「媒體驗證已通過」、video readyState 4、console error 0、1280px 無溢位；影片 `public/renders/render_1788047374290.mp4` 的 SHA-256 同為 `11be0932db1d8ce563b450bce9d0e1e8487001d38a290680aa48bb64c917f093`，H.264／AAC、1080×1920、30fps、25.045333 秒。內容 DB 雜湊未變、publish jobs／packages 0。source SHA `28655ee` 的 CI run `33281838905` 與 CodeQL run `33281838586` 均成功；本條最終證據的 docs-only commit 仍需再次等候其新 SHA checks。
